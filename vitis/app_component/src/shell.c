...
// === UART Helper ===
static void uart_get_line(char *buf, int max_len) {
    int i = 0;
    char c;
...
static void shell_help() {
    ...
    xil_printf("  task add <n>   - Add task n (1–4)\r\n");
}
...
static void shell_task_cmd(char* action, char* arg) {
    ...
    switch (id) {
        ...
        case 4: task_fn = button_task; break;
        default:
...
void shell_run() {
    ...
    xil_printf("\r\nRTOSsseract Shell Interface. Type 'help' for commands.\r\n");

    while (1) {
        xil_printf(">>> ");
        uart_get_line(cmd_buf, MAX_CMD_LEN);
